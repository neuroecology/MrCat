...
Usage: `RobustBiasCorr.sh` [--workingdir=<working dir>]
      ...
      [--forcestrictbrainmask={TRUE (default), FALSE}]
...
[[ -z $Base ]] && Base="$(basename $Input)" #Base=T"$Type"
...
# retrieve total bias field by dividing the original image by the final bias-corrected image
...
scale=$(fslstats $WD/${Base}_fast3_totbias -k $WD/${Base}_hpf_brain_mask2 -M | awk '{if ($1<1) print 1/$1; else print 1}')
$FSLbin/fslmaths $WD/${Base}_fast3_totbias -mul $scale -sub 1 $WD/${Base}_fast3_totbias
$FSLbin/fslsmoothfill -i $WD/${Base}_fast3_totbias -m $WD/${Base}_hpf_brain_mask2 -o $WD/${Base}_fast3_bias > /dev/null
$FSLbin/fslmaths $WD/${Base}_fast3_totbias -add 1 -div $scale $WD/${Base}_fast3_totbias
$FSLbin/fslmaths $WD/${Base}_fast3_bias -add 1 -div $scale $WD/${Base}_fast3_bias
