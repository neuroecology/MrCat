...
% Find pmin
[dpsim1 k11]=max(sum(S,1)); % datapoint farthest 

if dpsim1==-Inf pmin=NaN;
elseif strcmp(method,'bound')
    for k=1:N S(k,k)=-Inf; end;
    
    m=max(S,[],2);  % max of rows, best similarity ind => exemplar
    
    tmp=sum(m);     % sum of all best similarity ind - probably to determine the maximum possible value
    
    [yy ii]=min(m); % smallest of the best similarity ind
    
    tmp=tmp-yy-min(m([1:ii(1)-1,ii(1)+1:N]));   % sum(best) - smallest(best) - smallest(of the rest) 
    
    pmin=dpsim1-tmp;% 
    
    % max(sum) -- the datapoint farthest of all?
    % sum(max) -- sum of best similarities 
